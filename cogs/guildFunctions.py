...
class GuildFunctions(commands.Cog):
	...
	async def roleChannelMessage(self,ctx, index:int=None, *, message:str=""):
		...
		if ctx.guild.id not in self.bot.config["roleMsg"]:
			self.bot.config["roleMsg"][ctx.guild.id] = ["",""]
		self.bot.config["roleMsg"][ctx.guild.id][index-1]=message.replace("{best}","{0}").replace("{seiyuu}","{1}").replace("{asar}","{2}")
		utils.saveConfig(ctx)
		rxn = utils.getRandEmoji(ctx.guild.emojis, "hug")
		if rxn is None:
			rxn = "👍"
